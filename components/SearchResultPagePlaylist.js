  ...
  return (
    ...
    <div>
      <ul css={SearchResultPagePlaylistStyling}>
        {props.playlist.map((num, index) => <SearchResultPagePlaylistItem key={props.playlist[index].id} imgUrl={props.playlist[index].album.images[0].url}  index={index+1} songName={props.playlist[index].name} uri={props.playlist[index].uri}/>)}
      </ul>
    </div>
    </>
  );
